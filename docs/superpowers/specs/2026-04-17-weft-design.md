...
10. **Partition integrity is non-negotiable.** Weft must never silently remove a partition belonging to a preserved table. Pre-/post-deploy partition manifests provide forensic evidence. Policy changes that reduce the retention window require explicit opt-in.
11. **Bookmark preservation.** The `RefreshBookmark` property (written by "detect data changes" polling) is part of partition state and must be preserved on every preserved partition. Deliberate overrides exist for DB-restore and drift scenarios.
12. **Historical recovery.** A `restore-history` command exists to re-materialize historical partitions from the source system when data loss is detected, scoped by table and date range.
...
- `weft restore-history --target prod --table FactSales [--from YYYY-MM-DD --to YYYY-MM-DD]` — re-materialize historical partitions from source.
- `weft inspect partitions --target prod [--table X]` — list partitions and bookmarks on target (read-only utility).
...
- For altered tables: `alter` on the `Table` with new columns/measures, with the **target's current partition collection re-attached, including each partition's `RefreshBookmark` and all other partition-level properties**.
...
**INVARIANT (enforced):** `TmslBuilder` never emits a command that deletes a partition belonging to an existing, not-being-dropped table. After building, the builder walks its own output and asserts:

> For every table `T` in target not in `ChangeSet.TablesToDrop`, every partition name present in the target snapshot is either (a) present in the generated TMSL alter with its original properties intact, or (b) will be regenerated by the service via `ApplyRefreshPolicy` on an `IncrementalRefreshPolicy` table.

Violation throws `PartitionIntegrityException` and aborts before any I/O. A dedicated unit-test suite verifies this invariant against fixture inputs that attempt to trigger it (including malicious ChangeSets).
...
```
 ...
 4. Read target model
    ...
    - Write pre-deploy partition manifest:
      ./artifacts/<ts>-<target>-pre-partitions.json
      { table -> [ { name, refreshBookmark, modifiedTime, rowCount? } ] }
 ...
 6. Pre-flight checks
    ...
    - For each table with RefreshPolicyChanged=true:
        * Compute retained partition set under new policy
        * If new retained set excludes any partition present on target ->
          require --allow-history-loss AND profile.allowHistoryLoss=true.
          Otherwise abort with exit 6 and list affected partitions.
    - Run TmslBuilder invariant check on generated TMSL (partition deletes
      on preserved tables -> abort with exit 9).
...
12a. Post-deploy partition integrity gate
    - Re-read partition metadata from target (fast, metadata only)
    - Write post-deploy partition manifest:
      ./artifacts/<ts>-<target>-post-partitions.json
    - Diff pre vs. post manifests:
      For every table in pre AND NOT in TablesToDrop:
        * Any partition in pre but missing in post -> ABORT (exit 9).
          Deploy schema changes have been applied, but data integrity
          violated. Run on-failure hook. Caller must investigate before
          refresh. Command output includes the list of missing partitions
          and the absolute path to the pre-deploy manifest for recovery.
    - Bookmark drift detection (non-fatal):
        * Any preserved partition whose post-refreshBookmark differs from
          pre-refreshBookmark before any refresh ran -> WARN (indicates
          something else wrote to the model during deploy).
...
### 7A.7 Bookmark handling ("detect data changes")

When a policy's `PollingExpression` is configured, the Analysis Services engine stores a watermark on each partition as `Partition.RefreshBookmark`. On each refresh the engine evaluates the polling expression against the source; if the returned value matches the stored bookmark the partition is skipped, otherwise it is refreshed and the new value written back. This is why years-old historical partitions are cheap to refresh — the engine knows they haven't changed.

**Preservation guarantee.** `RefreshBookmark` is part of partition-level state and is copied verbatim whenever Weft preserves a target partition across an alter. This is covered by the §5.4 invariant and verified by unit tests that assert bookmark round-trip fidelity.

**Config knob for deliberate override:**
...
- `preserve` (default) — copy bookmarks as-is, honor detect-data-changes.
- `clearAll` — emit TMSL that clears every bookmark on refresh-scope tables before refresh runs. Forces the engine to re-evaluate every partition against the source on the next refresh. Use after a DB restore, replay, or when drift is suspected.
- `clearForPolicyChange` — clear bookmarks only on tables where `RefreshPolicyChanged=true`; all others preserved.

**CLI overrides:**
- `weft deploy --reset-bookmarks` — shortcut for `bookmarkMode=clearAll` for this run only.
- `weft deploy --reset-bookmarks=TableA,TableB` — scope to listed tables.
- `weft refresh --force` — refresh targeted partitions ignoring bookmarks.

**Explicitly out of scope:** Weft does NOT connect to the source database to evaluate the polling expression itself. Drift detection against the live source is a data-engineering concern requiring separate DB credentials; ops uses `--reset-bookmarks` deliberately when drift is known or suspected.

### 7A.8 Historical data protection

Incremental refresh has two distinct windows:

- **Rolling window** (`RollingWindowPeriods` × `RollingWindowGranularity`): the total history *retained* as partitions on the server.
- **Incremental window** (`IncrementalPeriods` × `IncrementalGranularity`): the portion *re-read on each refresh*.

Historical partitions outside the incremental window sit idle and never get refreshed. If anything deletes one, regular refreshes will not bring it back — only a policy re-application against a source system that still carries that history can.

**Threat model and defenses:**

| Risk | Defense |
|---|---|
| `ModelDiffer`/`TmslBuilder` bug drops a partition | §5.4 invariant, pre-emit validation, unit tests |
| `RefreshPolicyChanged` with shorter rolling window → service drops partitions on `ApplyRefreshPolicy` | §6 step 6 pre-flight: compute retained set under new policy, require `--allow-history-loss` + `profile.allowHistoryLoss=true` to proceed |
| Mid-deploy external interference | §6 step 4 pre-manifest + §6 step 12a post-manifest integrity gate, abort on drift |
| Manual TMSL run outside Weft | Outside Weft's control; manifests provide forensic record |

**Profile-level hard pin** (mirrors `allowDrops` pattern):

```yaml
profiles:
  prod:
    allowDrops: false
    allowHistoryLoss: false   # hard no; --allow-history-loss flag ignored
```

**Plan output when policy change would remove history:**

```
~ Table FactSales
  Policy change: RollingWindowPeriods 5y -> 3y
  WARNING: 2 historical partitions will be removed by ApplyRefreshPolicy:
           Year2020, Year2021
  Recovery if data is still needed: run `weft restore-history --table FactSales`
  BLOCKED: pass --allow-history-loss (and enable on profile) to proceed.
```

### 7A.9 `restore-history` command

Re-materialize historical partitions from the source system after data loss (e.g., an earlier deploy that allowed history loss, a manual incident, or a test environment being seeded).

```
weft restore-history
  --target prod
  --table FactSales
  [--from 2020-01-01]          # optional; defaults to rolling window start
  [--to   2022-12-31]          # optional; defaults to today
  [--clear-bookmarks=true]     # default true
...
**Flow:**
```
 1. Load config, resolve auth, read target.
 2. Assert --table exists on target and has RefreshPolicy != null.
    Otherwise exit 6.
 3. Compute partition set:
    - If --from/--to given: derive partition names covering that range
      under the policy's granularity structure.
    - Else: full rolling window per policy.
 4. Print recovery plan (partition list, estimated work).
 5. If --dry-run: exit 0.
 6. Pre-flight: assert model not currently refreshing.
 7. If --clear-bookmarks: emit TMSL that clears RefreshBookmark on each
    targeted partition (or all partitions if --from/--to omitted).
 8. Issue TMSL refresh command:
    - type: Full
    - applyRefreshPolicy: true
    - effectiveDate: <today or --effective-date>
    - scope: the target table
    The engine re-materializes partitions according to the policy's
    source expression with RangeStart/RangeEnd bound to each historical
    period.
 9. Poll progress; stream per-partition status to stdout.
10. Write ./artifacts/<ts>-<target>-restore-receipt.json:
    { table, requestedRange, partitionsBefore, partitionsAfter,
      bookmarksCleared, durations, result }
11. Exit 0 on success; 8 on refresh failure.
```

**Hard caveat, documented at the top of the command output and in `docs/troubleshooting.md`:**

> This command can only recover data the source system still has. If the warehouse/DW has purged history beyond the requested range, those partitions will re-materialize as empty or fail to build. Verify source-system retention before running restore-history over long ranges.
...
defaults:
  refresh:
    ...
    dynamicPartitionStrategy:
      ...
      newestN: 1
  allowDrops: false
  allowHistoryLoss: false          # hard pin when false, overrides CLI flag
  timeoutMinutes: 60
...
| Diff/validation errors | Drop without `--allow-drops`, history loss without `--allow-history-loss`, missing required param | 6 |
| TMSL execution errors | XMLA fault during deploy | 7 |
| Refresh errors | Table refresh fails, timeout | 8 |
| Partition integrity violation | TmslBuilder invariant failed pre-emit, or pre/post partition manifest diff shows unexpected deletion | 9 |
...
11. **Partition integrity invariant** (§5.4) — `TmslBuilder` never emits a partition delete on a preserved table. Validated pre-emit and again post-deploy via manifest diff. Two independent checks, one defense in depth.
12. **History-loss gate** (§7A.8) — policy changes that shrink the retained window are blocked by default; require explicit `--allow-history-loss` + profile-level opt-in to proceed. Plan output lists exactly what will be lost and directs the operator to `weft restore-history` as the recovery path.
13. **Pre-/post-deploy partition manifests** — always written to `./artifacts/`; form the forensic record and enable `restore-history` to operate from a known-good reference.
...
8. **Partition integrity invariant** (§5.4):
   - `TmslBuilder` rejects any ChangeSet that would emit a partition delete on a preserved table (synthetic malicious input).
   - `RefreshBookmark` survives alter → unit test loads target with bookmarks set, runs ChangeSet with schema change, asserts bookmarks intact in generated TMSL.
   - All partition-level properties (bookmark, query expressions, annotations, mode) round-trip on alter.
9. **Bookmark modes** — `preserve` / `clearAll` / `clearForPolicyChange` each produce the expected TMSL output.
10. **History-loss detection** — for a table with shrunken `RollingWindowPeriods`, pre-flight computes the correct set of partitions to be dropped by the service and blocks unless both flags set.
11. **Pre/post manifest diff** — given pre and post manifests with a partition missing on a preserved table, the integrity gate aborts with exit 9.
12. **`restore-history` partition-set computation** — given a policy and a date range, derives the expected partition-name structure (covering year/quarter/month boundaries correctly per granularity).
...
│   │   ├── Config/
│   │   ├── Hooks/
│   │   ├── Partitions/                  # manifest + integrity gate
│   │   ├── RefreshPolicy/               # policy diff, retention calc
│   │   ├── Restore/                     # restore-history partition-set logic
│   │   └── Weft.Core.csproj
│   │
│   ├── Weft.Xmla/                       # I/O layer
...
│   ├── incremental-refresh.md           # policy, bookmarks, history loss
│   ├── restore-history.md
│   ├── hooks.md
│   ├── troubleshooting.md
│   └── superpowers/specs/
